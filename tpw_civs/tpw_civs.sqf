...
if (count _this < 9) exitwith {hint "TPW CIVS incorrect/no config, exiting."};
...
tpw_civ_version = 1.17; // Version string
...
tpw_civ_fnc_civspawn =
	{
	private ["_civtype","_civ","_spawnpos","_i","_ct","_sqname"];
...
0 = [] spawn 
	...
	while {true} do
		{ 
		private ["_civarray","_deadplayer","_housestring","_uninhab"];
		...
		tpw_civ_houses = nearestObjects [position vehicle player,tpw_civ_habitable,tpw_civ_radius]; 
			...
			if (_uninhab) then 
				{
				tpw_civ_houses =tpw_civ_houses - [_x]
				}
		...
		// No houses to spawn at if player near exclusion object
			{
			if (_x distance position vehicle player < tpw_civ_radius) exitwith
				{
				tpw_civ_exclude = true;
				tpw_civ_houses = [];
				};	
			} foreach tpw_civ_excarray;
	...
	if (tpw_civ_active) then
		...
		if (tpw_civ_exclude) then
			...
			if !(alive _x) then 
				...
				if (_x distance vehicle player > _deleteradius && ((lineintersects [eyepos player,getposasl _x,player,_x]) || (terrainintersectasl [eyepos player,getposasl _x]))) then
					{
